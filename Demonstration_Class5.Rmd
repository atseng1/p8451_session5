...
# For categorical data, "model.matrix" creates indicator variables for you. It puts the data in the format you need for algorithms. Removing the outcome variable with -1.
...
plot(model.1.cv)
# Gives you minimum value of lambda
model.1.cv$lambda.min
# Gives you first standard error of lambda
...
set.seed(100)
# LASSO
model.lasso.cv = cv.glmnet(x.train, life.exp.train, alpha = 1)
plot(model.lasso.cv)
model.lasso.cv$lambda.min
model.lasso.cv$lambda.1se

model.lasso.train.final = glmnet(x.train, life.exp.train, alpha = 1, lambda = model.lasso.cv$lambda.min)
coef(model.lasso.train.final)

model.lasso.cv.pred = model.lasso.train.final %>% predict(x.test) %>% as.vector()

data.frame(RMSE=RMSE(model.lasso.cv.pred, life.exp.test), RSQ=R2(model.lasso.cv.pred, life.exp.test))



set.seed(100)
# Elastic net
model.en.cv = cv.glmnet(x.train, life.exp.train, alpha = 0.5)
plot(model.en.cv)
model.en.cv$lambda.min
model.en.cv$lambda.1se

model.en.train.final = glmnet(x.train, life.exp.train, alpha = 0.5, lambda = model.en.cv$lambda.min)
coef(model.lasso.train.final)

model.en.cv.pred = model.en.train.final %>% predict(x.test) %>% as.vector()

data.frame(RMSE=RMSE(model.en.cv.pred, life.exp.test), RSQ=R2(model.en.cv.pred, life.exp.test))
```

The LASSO model gives a lower RMSE value than the elastic net model, hence I would select the LASSO model for this study.
...
### Exercise: The following code will allow you to fix the alpha (I have it set to 0 for a ridge) and run either a ridge or lasso analysis. Use that code to run both ridge and Lasso using the caret package and obtain coefficients and evaluation metrics. Note: you would never fix the lambda and vary the alpha.

```{r}
# Create grid to search lambda. We want to try 100 different lambdas.
...
# Make predictions
ridge.pred = model.ridge %>% predict(x.test)
...
model.lasso = train(
  ...
  # expand.grid creates a matrix space and finds the best combination in the space
...
# Make predictions
lasso.pred = model.lasso %>% predict(x.test)

# Model prediction performance
data.frame(
  RMSE = RMSE(lasso.pred, test.data$life_exp),
  Rsquare = R2(lasso.pred, test.data$life_exp))
```

The LASSO model has a lower RMSE value than the ridge model.
